...
int main()
{
  ...
  test_residual Res;                           // Create residual
  Vector<double> x_0(2, 0.5);                        // Initial guess
  x_0[1] = 0.25;
  cout << " * x_0 = " << x_0 << endl;
  Newton<double> newton( &Res );               // Create Newton object
  ...
  Arc_problem arc_prob;
  arc_prob.p = 1.0;

  const double tol = 1.e-6;
  // Scalar newton iteration problem
  Newton<double> newton_arc( &arc_prob, 8, tol );
  ...
  Polynomial<double> f( std::vector<double> { -1.0, 2.0, 1.0, 1.0 } );
